...
export async function createVendorLocationAction(
  vendorId: string,
  ...
  try {
    ...
    if (isDuplicateKeyError(err)) {
      return { error: "A location with that code already exists for this vendor." };
    }
    throw err;
  }

  revalidatePath(`/vendors/${vendorId}/locations`);
  redirect(`/vendors/${vendorId}/locations`);
}
