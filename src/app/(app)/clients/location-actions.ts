...
import { createLocation, updateLocation } from "@/server/client-locations";
...
/**
 * Edit a location. requireTenant (per-record, like createLocationAction). Calls updateLocation,
 * which audits client_location.updated. Bind both ids: updateLocationAction.bind(null, clientId, locationId).
 */
export async function updateLocationAction(
  ...
  if (!name) return { error: "Name is required." };
  if (!addressLine1 || !city || !stateProvince || !postalCode) {
    return { error: "Address line 1, city, state/province, and postal code are required." };
  }
  ...
  try {
    await updateLocation({
      ...
      patch: { name, locationCode, addressLine1, addressLine2, city, stateProvince, postalCode, country },
    });
  } catch (err) {
    if (err instanceof Error && err.message === "CLIENT_LOCATION_NOT_FOUND") {
