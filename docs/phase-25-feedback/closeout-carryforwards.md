# Phase 25 — Carry-Forwards

The canonical post-MVP backlog, rolled forward from
`docs/phase-24-observability/closeout-carryforwards.md`, with the new Phase-25 items added.
**Phase 25 RETIRES NOTHING** from the inherited bank — it is a net-new build phase (feedback loop:
harvest operator corrections → few-shot). Verified VERBATIM against the live Phase-24 bank text: no
inherited backlog item is *delivered* by Phase 25 (roadmap §6 says feedback "harvests corrections" —
it does not satisfy any banked CF item). So no retirement or discharge of an inherited item is
claimed. Four Phase-25-internal items are newly opened (CF-25.1–25.4); all detailed below.

## Discharged / resolved since the last bank (Phase 25)

**Phase 25 discharged no INHERITED backlog item** — it built a net-new harvesting reader + few-shot
injection seam + a phase-blocking measurability harness; nothing from the prior bank was delivered.
(B-16.5 "LLM-assisted draft phrasing" is retired **per agent by Phase 26**, not by Phase 25 — Phase 25
feeds existing agents, it does not add one. B-16.5 stays OPEN here.)

## New Phase-25 banked items (open)

| Id | Item | What's needed | Why deferred |
|---|---|---|---|
| **CF-25.1** | **Few-shot provenance not recorded on `agent_runs`** — `prompt_version` records which template ran, but NOT which correction examples were injected into that run, so provenance is incomplete about what the model actually saw. | Record the injected-pair set (or a hash/count) on the run — likely a new `agent_runs` column or a side table. | Observability concern, deferred from the 25a schema-need assessment. No schema added in Phase 25 (`0047` left free for the deciding phase). |
| **CF-25.2** | **Human-curation "approved-for-few-shot" flag not built** — every harvested gold/positive pair is eligible for injection; no operator step to bless/exclude specific corrections. | A curation surface + a flag (and the schema it implies) on the review/draft, plus selection heuristics that consume it. | With single-digit live pairs there is no curation problem to solve yet. Deferred until selection heuristics are real / data is non-trivial. |
| **CF-25.3** | **Negatives (rejects) harvested but not injected** — the reader classifies + returns NEGATIVE pairs, but `selectFewShotPairs` excludes them from the injectable set. | A contrastive-eval rung that uses rejects as "what NOT to produce" signal (or negative few-shot). | A reject is not a "good output" example; MVP injects gold+positive only. Banked for contrastive-eval use later. |
| **CF-25.4** | **Held-out measurement is seeded-synthetic-only; feedback-poison unaddressed** — the phase-blocking harness proves plumbing + a discriminating metric on a SEEDED corpus, not on live corrections (1 gold pair platform-wide). No trust filter on which corrections are injectable. | A live held-out eval once review tables fill; and a trust/quality filter on corrections when the operator pool grows. | Live data is too thin to measure a real lift today; the trusted-operator set is implicit signal quality for now. Roadmap §25: the "feedback poison" concern (untrusted corrections) revisits as the operator pool grows. |

## Forward-pointer (NOT a retirement) — rolls forward OPEN

- **CF-24.2 — Live autonomy trigger.** `autoDispatchDraftForJob` is invoked by nothing in app code
  (no job-creation hook / cron / queue). Phase 25 did **NOT** wire it — correctly out of scope (this
  phase is the feedback loop, not the autonomy trigger). The governed auto-dispatch (Phase 23) +
  observability evidence (Phase 24) exist; the live trigger remains a discrete, evidence-informed
  future decision (§2.3 — permission ≠ readiness). **Rolls forward OPEN, unchanged.**

---

## Inherited (roll forward, UNCHANGED — from the Phase-24 bank)

### Phase-24 banked items (open)

| Id | Item | What's needed | Why deferred |
|---|---|---|---|
| **CF-24.2** | **Live autonomy trigger** — `autoDispatchDraftForJob` is invoked by nothing in app code; no job-creation hook / cron / queue calls it. The governed auto-dispatch (Phase 23) + the observability evidence (Phase 24) exist, but the trigger is unwired. | A deliberate decision + wiring to invoke `autoDispatchDraftForJob` (e.g. on job creation) for a **proven** agent on a **proven** tenant, informed by the `/agents` evidence; plus a first-tenant enablement (`autonomyEnabled:true` + cleared guardrails). | **§2.3 — permission ≠ readiness.** Phase 24 deliberately built the evidence surface and stopped short of flipping the switch. Phase 25 did not touch it. |

**Phase-24 dependency note (NOT a retirement):**
- **CF-23.1's multi-provider-wiring dependency is SATISFIED** — Phase 24 built the provider registry +
  direct-provider path + failover + per-policy preference. **CF-23.1 itself stays OPEN**: it still
  needs **tenant-supplied encrypted key storage (CF-12.4)** + a Settings UI; Phase 24 used the
  **platform's** env keys only. Rolled forward open, one of its two dependencies discharged.

**Phase-24 soft notes (open, low-priority):**
...
18/22/28 **as the surfaces land**)." Phases 22/23/24/25 built none of those PM/snow/mass-op operator
UIs — they remain **unfulfilled** and roll forward OPEN. Because §9's wording is **conditional** ("as
the surfaces land"), this is **not** a false flat retirement claim; the standing §6/§9 over-attribution
watchpoint carries forward.

### Phase-23 banked items (open)
| Id | Item | Status |
|---|---|---|
| **CF-23.1** | **Tenant-supplied LLM API keys + self-service AI restrictions in Settings** — per-tenant **encrypted key storage** (new security surface) + multi-provider wiring to consume tenant keys + a Settings UI. "Other agent restrictions" beyond on/off = the **Phase-28 condition vocabulary**. | OPEN. **Multi-provider-wiring dependency satisfied by Phase 24**; still needs **CF-12.4** (credential encryption-at-rest) + the Settings UI. (The encryption item is **CF-12.4**, not CF-12.1 — live bank wins over the 23h handoff prose.) |
...
- **`autonomyEnabled`-naming clarity.** `ResolvedPolicy.autonomyEnabled` reflects only the **policy +
  ...
  withinSpendCeilings().ok` at the enforcement site. A rename or a derived `fullyPermitted()` helper could clarify.
...
| B-16.5 | LLM-assisted draft phrasing (provider seam + `ai_prompt_templates`). **Retired per agent by Phase 26**, not Phase 25 — stays OPEN. |
...
| B-15.3 | Mass-op operator UI + snow operator screens. *(§9 "Phases 18/22/28" bucket — unfulfilled by Phases 22/23/24/25.)* |
...
| Id | Item |
|---|---|
| B-14.1 | PM Programs UI placement (dedicated section + client-profile read list). *(§9 "Phases 18/22/28" bucket — unfulfilled by Phases 22/23/24/25.)* |
| B-14.2 | Live cron / scheduler trigger (timer calling `runDueSchedules`). |
| B-14.3 | Per-location scope/trade override on a PM membership. *(§9 bucket — unfulfilled by Phases 22/23/24/25.)* |
| B-14.4 | Mass-dispatch + generic mass-update UI (operator-portal). *(§9 bucket — unfulfilled by Phases 22/23/24/25.)* |
...
| CF-14.3 | PM program/schedule CRUD UI. *(§9 "Phases 18/22/28" bucket — unfulfilled by Phases 22/23/24/25.)* |
...
- **MariaDB-JSON parse-at-read** — `json()` columns come back as strings; parse at the read boundary.
  Read verdicts from a file + true exit. **(Phase-25 corollary: to read a `json()` column as its RAW
  stored string, select via `CAST(col AS CHAR)` to bypass drizzle's `json()` decoder — used by the
  scope correction-pairs reader so the JSON is handed to the few-shot layer un-double-encoded.)**
...
- **Migration cadence** — sandbox apply → `-E` contract-verify → prod-confirm gate → prod apply; a
  ...
  resolved DB name before any prod DDL. (Phases 24 & 25 added no migration; 0047 untouched.)
...
- **Harness teardown under `FK_CHECKS=0` does NOT cascade** — delete children explicitly by tracked id;
  **never delete by a `created_at`/timestamp window** (JS-Date-vs-DB-timezone skew). Re-applied in the
  Phase-25 harness (children-first by tracked id, fresh `phase25-harness-tenant`).
...
- **`agent_decisions` requires a synthetic `agent_runs` row (Phase-23)** — `agent_run_id` NOT NULL, no
  direct `agent_id`; a rule-based agent that wants a decision row must `openRun` first. **(Phase-25
  corollary: correction drafts/reviews also hang off a real `agent_runs` row — the harvest reader joins
  `agent_runs → drafts → reviews`; the harness seeds the full chain, never orphaned reviews.)**
...
- **Standalone TS scripts need `export {};` (Phase-24 / CF-24.1)** — a bare script (no top-level
  ...
  `main()` MUST start with `export {};` (module isolation). Re-applied in `check-phase-25.ts`.
- **Prod-ops scripts follow the `db/seeds` precedent, NOT the check-script sandbox guard (Phase-24)** —
  ...
  check-`*.ts` rewrite-to-`_sandbox`-or-refuse guard. Its safety is **dry-run-default + loud target-DB
  print + explicit `--apply`**. Only **check harnesses** force `_sandbox` (e.g. `check-phase-25.ts`).
- **Multi-provider keys are PLATFORM env keys (Phase-24)** — `ANTHROPIC_API_KEY` / `OPENAI_API_KEY`;
  ...
  ran (truthful cost/volume under failover). **(Phase-25 corollary: `PROVIDER_REGISTRY` is mutable
  DATA — a check harness may override `.buildModel` to inject a `MockLanguageModelV3`, but MUST restore
  the real factory in `finally` to avoid cross-test contamination.)**
- **`createdAt` is THE canonical latest-review-per-draft ordering (Phase-25)** — the feedback harvest
  reader and the Phase-24 observability reader BOTH dedupe to the newest review per draft by
  `created_at`, via the ONE shared `latestReviewPerDraft` primitive (extracted from
  `agent-observability.ts` into `correction-pairs.ts`). The two readers MUST NOT diverge on this
  ordering or the correction-bucket counts would disagree with the observability approve-as-is numbers.
  The shared-primitive extraction is behavior-preserving — gated by the Phase-24 harness staying 28/0.
- **Roadmap §6/§9 over-attribute retirements** — CF-19.4 (CF-12), CF-20.3 (CF-13.4), CF-21.1 (B-16.3 —
  ...
  unfulfilled by Phases 22/23/24/25, no correction needed, but watch it. **Verify any "retires/depends-on
  X" claim against the live bank text** (live bank wins over roadmap §6/§9 and over handoff prose — e.g.
  the encryption item is **CF-12.4**, not CF-12.1; and B-16.5 retires by **Phase 26**, not Phase 25).
